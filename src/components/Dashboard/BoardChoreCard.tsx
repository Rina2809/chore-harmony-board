...
const BoardChoreCard = ({ chore, onEdit, onToggleComplete, isInCompletedColumn }: BoardChoreCardProps) => {
  ...
  return (
    <div className={`${cardBgColor} rounded-lg border border-gray-200 p-3 shadow-sm hover:shadow-md transition-shadow cursor-pointer group relative`}>
      ...
      <div className="flex items-center justify-between ml-2">
        <div className="flex-1 min-w-0">
          <div className="flex items-center space-x-3">
            ...
            {/* Chore title */}
            <div className="flex-1 min-w-0">
              <h3 className={`font-medium text-gray-900 truncate ${isInCompletedColumn ? 'line-through text-green-700' : ''}`}>
                {chore.title}
              </h3>
            </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 p-1 hover:bg-gray-100 rounded flex-shrink-0">
